...
let cepArray = localStorage.getItem("cepList")
  ? JSON.parse(localStorage.getItem("cepList"))
  : [];
...
function criarElemento(modelo) {
  ...
  cellDeletar.appendChild(
    criaButtonGeneric("Deletar", "btn_deletar", `deletar('${rowId}')`)
  );
...
function efetuaGravacao() {
  ...
  let objExiste = cepArray.find(
    (o) =>
      o.cep.trim() === inputCep.trim() &&
  ...
  model.id =
    cepArray === undefined || cepArray === null || cepArray.length === 0
      ? 0
      : cepArray.length + 1;
...
function deletar(rowId) {
  ...
  var buttonSimModalPergunta = document.getElementById("simModalPergunta");
  buttonSimModalPergunta.setAttribute(
    "onclick",
    `efetuaExclusao('${rowId}', ${objEndereco.cep})`
  );
